...
# Funzione helper per la pausa
def wait_for_next_step(test_name):
    """Mette in pausa l'esecuzione e attende l'input dell'utente."""
    input(f"\n--- Test '{test_name}' completato. Premi Invio per continuare con il prossimo test... ---")
...
def test_create_routine_actionTime(entities_url, auth_token, created_roller_shutters_list):
    ...
    try:
        ...
        try:
            ...
            # ... (altre asserzioni su rollerShutters se necessario)
    ...
    # --- PAUSA ---
    wait_for_next_step("test_create_routine_actionTime")
...
def test_create_routine_lightSensor(entities_url, auth_token, created_light_sensor, created_roller_shutters_list):
    ...
    # --- PAUSA ---
    wait_for_next_step("test_create_routine_lightSensor")
...
def test_delete_routine(entities_url, auth_token, created_actionTime_routine):
    ...
    # --- PAUSA ---
    wait_for_next_step("test_delete_routine")

'''
# Il codice commentato non verrà eseguito, quindi non serve la pausa qui.
def test_patch_routine_name(entities_url, auth_token, created_actionTime_routine):
    # ... codice ...
    # --- Se decommenti questo test, aggiungi la pausa qui ---
    # wait_for_next_step("test_patch_routine_name")
'''
...
def test_patch_routine_rollerShutters(entities_url, auth_token, created_actionTime_routine, created_roller_shutters_list):
    ...
    try:
        ...
        returned_names = {s.get("name") for s in linked_shutters if isinstance(s, dict)}
    ...
    # --- PAUSA ---
    wait_for_next_step("test_patch_routine_rollerShutters")
...
def test_patch_routine_actionTime(entities_url, auth_token, created_actionTime_routine):
    ...
    new_time_to_send = "11:45:00"
    expected_time_in_response = "11:45"
    ...
    # --- PAUSA ---
    wait_for_next_step("test_patch_routine_actionTime")
...
def test_patch_routine_lightSensor(entities_url, auth_token, created_lightSensor_routine, another_created_light_sensor):
    ...
    try:
        ...
        assert linked_sensor.get("name") == new_sensor_name
    ...
    # --- PAUSA ---
    # Non è strettamente necessaria dopo l'ultimo test, ma la aggiungo per coerenza.
    # Puoi rimuoverla se preferisci che lo script termini immediatamente dopo l'ultimo test.
    wait_for_next_step("test_patch_routine_lightSensor")