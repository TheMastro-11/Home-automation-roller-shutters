...
def test_create_routine_actionTime(entities_url, auth_token, created_roller_shutters_list):
    ...
    expected_time_in_response = "10:30" # Formato HH:MM visto nella risposta precedente

    # --- CORREZIONE: Usa di nuovo la chiave "time" nel payload inviato ---
    # L'errore backend "Unrecognized field 'actionTime'" indica che il DTO Java si aspetta "time".
    payload = {
        "name": routine_name,
        "time": routine_time_to_send, # Usa la chiave "time" qui!
    ...
    print(f"[Test] Payload: {payload}") # Assicurati che qui appaia "time"

    try:
        # Ora la richiesta dovrebbe essere deserializzata correttamente dal backend
